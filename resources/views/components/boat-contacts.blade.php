        ...
        <div class="relative max-w-[340px] bg-white shadow-lg rounded-lg">
            ...
            <header class="pt-6 pb-4 px-5 border-b border-gray-200">
                <div class="flex justify-between items-center mb-3">
                    ...
                    <div class="relative inline-flex shrink-0">
                        <a class="text-gray-400 hover:text-gray-500 rounded-full focus:ring-0 outline-hidden focus:outline-hidden" href=" {{ route('boat.fisher.edit') }}">
            ...
            <!-- Card body -->
            <div>
                <h3 class="pt-3 pl-5 pr-5 text-xs font-semibold uppercase text-gray-400 mb-1">Fishing net</h3>
                        ...
                        <div id="net_contacs" @add-fisher-net="addNetOnBroadcast(event);" x-data="net" x-init="loadContacts()">
                            ...
                            {{-- @foreach ($net as $fisher) --}}
                                <div :id="['fisher_' + contact.id]"  
                                    ...
                                    >
                                    <!-- User -->
                                    <button @click.prevent="showChat=true; $nextTick(() => { $refs.input_message.focus(); });" class="w-full text-left py-2 focus:outline-hidden focus-visible:bg-indigo-50">
                                        <div class="flex items-center">

                                            <img class="rounded-full items-start shrink-0 mr-3" :src="contact.nick_image" width="32" height="32" :alt="contact.name" />
                                            <div>
                                                <h4 class="text-sm font-semibold text-gray-900" x-text="contact.name"></h4>
                                                <div class="text-[13px]">The video chat ended · 2hrs</div>
                                            </div>
                                        </div>
                                    </button>  
                                    <x-boat-chat :user="$user"/>
                                </div>
                            {{-- @endforeach --}}
                            </template>
                        </div>
            ...
            <!-- Bottom right button -->
            {{-- <button wire:click="add_contact" class="absolute bottom-5 right-5 inline-flex items-center text-sm font-medium text-white bg-indigo-500 hover:bg-indigo-600 rounded-full text-center px-3 py-2 shadow-lg focus:outline-hidden focus-visible:ring-2">
